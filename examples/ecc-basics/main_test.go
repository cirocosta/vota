...
func TestDemonstrationLawsHold(t *testing.T) {
	result, err := demonstrate(newHashReader("ecc-basics"))
	if err != nil {
		t.Fatal(err)
	}
	if !result.sharedEqual || !result.additionEqual {
		t.Fatalf("result = %+v", result)
	}
	if result.alicePublic == result.bobPublic {
		t.Fatal("independent public keys are equal")
	}
}
